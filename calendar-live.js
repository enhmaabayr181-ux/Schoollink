...
async function shRenderCalendar(){
  ...
  let eventQuery=sb.from('school_events').select('*').eq('school_id',membership.school_id).gte('starts_at',from).lte('starts_at',to).order('starts_at');
  if(membership.role==='admin')eventQuery=eventQuery.eq('created_by',session.user.id);
...
window.shCalDelete=async eventId=>{if(!confirm('Энэ арга хэмжээг устгах уу?'))return;const {error}=await sb.from('school_events').delete().eq('id',eventId).eq('created_by',session.user.id);if(error)return alert(error.message);await shRenderCalendar()};
