...
const SH_PLANS={monthly:{label:'Сарын эрх',amount:19900}};
...
async function shRenderBilling(){
  ...
  <div class="card wide"><div class="sectionTitle"><h3>Багц сонгох</h3><span class="pill">Эцэг эх үнэгүй</span></div><div class="shPlanGrid">${planCard('monthly',SH_PLANS.monthly,true)}</div><div id="shPaymentStatus" class="status"></div></div>
...
function shBillingInit(){if(shBillingReady)return;shBillingReady=true;const prev=setRole;setRole=function(role){prev(role);setTimeout(shBindBillingNav,80)};const obs=new MutationObserver(()=>setTimeout(shBindBillingNav,0));if($('nav'))obs.observe($('nav'),{childList:true});setInterval(shBindBillingNav,1600);setTimeout(shBindBillingNav,500);const style=document.createElement('style');style.textContent=`.shPlanGrid{display:grid;grid-template-columns:minmax(0,420px);gap:12px}.shPlanCard{margin:0}.shPlanCard.featured{outline:2px solid rgba(115,87,255,.25);background:#f7f4ff}.shPlanPrice{font-size:28px;font-weight:900;letter-spacing:-1px;margin:14px 0}@media(max-width:760px){.shPlanGrid{grid-template-columns:1fr}.shPlanPrice{font-size:26px}}`;document.head.appendChild(style)}
