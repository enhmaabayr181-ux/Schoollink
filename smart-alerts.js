(() => {
  ...
  style.textContent=`
    ...
    .shAlertsHead{display:flex;align-items:center;justify-content:space-between;gap:12px;margin:0 0 13px;width:100%;border:0;background:transparent;padding:0;color:inherit;cursor:pointer;text-align:left}.shAlertsHead h3{margin:0;font-size:20px}.shAlertsHeadRight{display:flex;align-items:center;gap:9px}.shAlertBadge{background:#f0ecff;color:#6349e8;border-radius:999px;padding:6px 10px;font-size:12px;font-weight:800}.shAlertChevron{color:#765ff2;font-size:18px;font-weight:900;transition:transform .2s}.shAlerts.collapsed .shAlertList{display:none}.shAlerts.collapsed .shAlertsHead{margin-bottom:0}.shAlerts.collapsed .shAlertChevron{transform:rotate(-90deg)}
  ...
  function render(){
    ...
    const collapsed=localStorage.getItem('schoolhub-alerts-collapsed')==='1';card.classList.toggle('collapsed',collapsed);card.innerHTML='<button type="button" class="shAlertsHead" aria-expanded="'+(!collapsed)+'"><h3>🔔 Ухаалаг анхааруулга</h3><span class="shAlertsHeadRight"><span class="shAlertBadge">'+data.length+' сануулга</span><span class="shAlertChevron">⌄</span></span></button><div class="shAlertList">'+data.map(a=>'<button type="button" class="shAlert '+a[0]+'" data-sh-alert-jump="'+esc(a[4])+'"><span class="shAlertIcon">'+a[1]+'</span><span class="shAlertText"><b>'+esc(a[2])+'</b><span>'+esc(a[3])+'</span></span><span class="shAlertGo">›</span></button>').join('')+'</div>';
  }
  document.addEventListener('click',e=>{
    const head=e.target.closest('.shAlertsHead');if(head){const card=head.closest('.shAlerts'),collapsed=!card.classList.contains('collapsed');card.classList.toggle('collapsed',collapsed);head.setAttribute('aria-expanded',String(!collapsed));localStorage.setItem('schoolhub-alerts-collapsed',collapsed?'1':'0');return}
